...
def checkout(request):
    ...
    order_items = order_qs[0].orderitmes.all()
...
def payment(request):
    ...
    store_id = 'abc61e82cdd29f5b'
    API_key = 'abc61e82cdd29f5b@ssl'
    ...
    order_qs = Order.objects.filter(user=request.user, ordered=False)
    order_items = order_qs[0].orderitmes.all()
    order_items_count = order_qs[0].orderitmes.count()
    order_total = order_qs[0].get_totals()
    ...
    #return render(request,'App_Payment/payment.html',context={'saved_address':saved_address})
...
def complete(request):
    if request.method == 'POST' or request.method == 'post':
        ...
        if status == 'VALID':
            ...
            messages.warning(request,f'Payment failed! try again! Page will be redirected')
